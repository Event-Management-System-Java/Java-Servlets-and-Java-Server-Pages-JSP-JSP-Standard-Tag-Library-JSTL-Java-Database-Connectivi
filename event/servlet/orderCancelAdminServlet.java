...
/**
 * Servlet implementation class orderCancelAdminServlet
 */
@WebServlet("/orderCancelAdminServlet")
public class orderCancelAdminServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public orderCancelAdminServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			if(request.getSession(false).getAttribute("userEmail") != null ) {
				...
				order.setRefund(Integer.parseInt(request.getParameter("refund")));
				
				orderCancelService cancelService = new orderCancelService();
				cancelService.orderCancelAdmin(order);
				...
				}else if(cancelService.getSuccess()==2) {
					HttpSession session = request.getSession();
